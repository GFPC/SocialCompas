...
class BotHandlers:
    ...
    Supports both native MAX Bot API v2 updates and Telegram-style simulator updates.
    """
    ...
    async def handle_update(self, update: Dict[str, Any]) -> Tuple[Optional[str], Optional[str], Optional[Dict[str, Any]]]:
        """
        Processes an incoming update dict.
        Returns a tuple: (chat_id, response_text, keyboard_markup)
        """
        update_type = update.get("update_type", "")

        # 1. MAX Native Event: bot_started
        if update_type == "bot_started":
            chat_id = str(update.get("chat_id") or update.get("user_id"))
            return await self.handle_command(chat_id, "/start", "")

        # 2. MAX Native Event: message_callback or Telegram-style callback_query
        if update_type == "message_callback" or "callback_query" in update:
            if "callback" in update:
                cb = update["callback"]
                data = cb.get("payload", cb.get("data", ""))
                chat_id = str(update.get("message", {}).get("recipient", {}).get("chat_id") or update.get("chat_id") or cb.get("user", {}).get("user_id"))
                if "callback_id" in cb:
                    try:
                        await self.api.answer_callback_query(cb["callback_id"])
        ...
        # 3. MAX Native Event: message_created or standard message
        message = update.get("message", {})
        if not message and "text" in update:
            message = update

        # Extract chat_id
        recipient = message.get("recipient", {})
        sender = message.get("sender", {})
        chat_id = str(recipient.get("chat_id") or update.get("chat_id") or sender.get("user_id") or message.get("chat", {}).get("id", "default_chat"))
        ...
        return chat_id, f"🤖 Вы написали: *{text}*\n\nВоспользуйтесь кнопками ниже для работы с ботом-помощником:", self.get_main_menu_keyboard()
    ...
    async def handle_command(self, chat_id: str, command: str, args: str) -> Tuple[str, str, Dict[str, Any]]:
        ...
        if command in ("/start", "/menu"):
            ...
            text = (
                ...
                "• Авторизация: Токен от @MasterBot / business.max.ru\n"
                "• Статус: Демонстрационный бот готов к работе\n"
